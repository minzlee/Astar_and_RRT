...
if __name__ == "__main__":
    ...
    RaveInitialize()
    RaveLoadPlugin('BiRRTplugin/build/BiRRTplugin')
    RRTModule = RaveCreateModule(env,'BiRRTModule')
    ...
    with env:
        ...
        # robot.GetController().SetDesired(robot.GetDOFValues());
        # goalconfig = [-0.16,0.1,-1.008,0,0,-0.11,0]
        ### YOUR CODE HERE ###
        ###call your plugin to plan, draw, and execute a path from the current configuration of the left arm to the goalconfig
        strStart = str(startconfig).replace(",", "")[1:-1]
        strGoal  = str(goalconfig).replace(",", "")[1:-1]
        RRTModule.SendCommand("passEnvRobot")
        RRTModule.SendCommand("setStartConfig " + strStart)
        RRTModule.SendCommand("setGoalConfig " + strGoal)
        RRTModule.SendCommand("setGoalBias 0.11")
        RRTModule.SendCommand("getRRTPath")
        ### END OF YOUR CODE ###
    waitrobot(robot)
